...
function renewContent() {
	...
	if(window.confirm('Bạn có muốn khôi phục dữ liệu lần lưu cuối?')) {
		var data = loadUserdata('home');
		if(in_array((data = trim(data)), ['', 'null', 'false', null, false])) {
			parent.showDialog('Không có dữ liệu để phục hồi!');
			return;
		}
...
function format(type, para){
	...
	if(!gIsIE){
		switch(type){
			case "Cut":
				sAlert = "Trình duyệt không cho dùng chức năng này. Để Cut chọn đối tượng và ấn Ctrl+X";
				break;
			case "Copy":
				sAlert = "Trình duyệt không cho dùng chức năng này. Để Copy chọn đối tượng và ấn Ctrl+C";
				break;
			case "Paste":
				sAlert = "Trình duyệt không cho dùng chức năng này. Để dán nội dung ấn Ctrl+V";
				break;
		}
	}
	if(sAlert != ""){
...
function doodleBox(event, id) {
	if(parent.$('uchome-ttHtmlEditor') != null) {
		...
		alert("Find graffiti board initialization data");
	}
}
...
function fDisplayElement(element,displayValue) {
	if(gIEVer<=5.01 && gIsIE){
		alert('Chỉ hỗ trợ phiên bản cao hơn IE 5.01');
		return;
	}
	fHideMenu();
...
function changeEditType(flag, ev){
	...
	try{
		...
		if(flag){
			...
			ev = ev || event;
			if(ev){
				if(window.confirm("Chuyển đổi sẽ mất một số định dạng văn bản.\nBạn muốn tiếp tục?")){
