...
function addFormEvent(formid, focus){
	...
	formNode[stmp[1]].onblur = function () {
		if(formNode[stmp[1]].value == '') {
			errormessage(formNode[stmp[1]].id, 'Vui lòng điền mật khẩu');
		}else{
	...
	formNode[stmp[3]].onblur = function () {
		if(formNode[stmp[3]].value == '') {
			errormessage(formNode[stmp[3]].id, 'Nhập địa chỉ email');
		}
...
function checkusername(id) {
	...
	if(username.match(/<|"/ig)) {
		errormessage(id, 'Tên người dùng chứa các ký tự bị cấm');
		return;
	}
	var unlen = username.replace(/[^\x00-\xff]/g, "**").length;
	if(unlen < 3 || unlen > 15) {
		errormessage(id, unlen < 3 ? 'Tên người dùng ít hơn 3 kí tự' : 'Tên người dùng lớn hơn 15 kí tự');
		return;
	}
	var x = new Ajax();
	$('tip_' + id).parentNode.className = $('tip_' + id).parentNode.className.replace(/ p_right/, '');
...
function checkpassword(id1, id2) {
	...
	errormessage(id2);
	if($(id1).value != $(id2).value) {
		errormessage(id2, 'Hai mật khẩu không khớp nhau');
	} else {
