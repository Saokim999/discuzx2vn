...
function checklength(theform) {
	...
	showDialog('Độ dài: ' + mb_strlen(message) + ' ký tự,' + (postmaxchars != 0 ? 'Hệ thống giới hạn: ' + postminchars + ' đến ' + postmaxchars + ' ký tự.' : ''), 'notice', 'Word Check');
}
...
function validate(theform) {
	...
	if(ispicstyleforum == 1 && ATTACHORIMAGE == 0 && isfirstpost) {
		showError('Tải lên ít nhất một hình ảnh làm trang bìa');
		return false;
	}
...
function updateAttach(aid) {
	...
	if(!objupdate.innerHTML) {
		...
		objupdate.innerHTML = '<input type="file" name="attachupdate[paid' + aid + ']"><a href="javascript:;" onclick="updateAttach(' + aid + ')">Huỷ</a>';
	} else {
		obj.style.display = '';
		objupdate.innerHTML = '';
	}
...
function attachoption(type, op) {
	if(!op) {
		...
		showDialog('<div id="unusedwin" class="c altw" style="overflow:auto;height:100px;">' + $('unusedlist_' + type).innerHTML + '</div>' +
			'<p class="o pns"><span class="z xg1"><label for="unusedwinchkall"><input id="unusedwinchkall" type="checkbox" onclick="attachoption(\'' + type + '\', 3)" checked="checked" />Chọn tất</label></span>' +
			'<button onclick="attachoption(\'' + type + '\', 1);hideMenu(\'fwin_dialog\', \'dialog\')" class="pn pnc"><strong>OK</strong></button></p>', 'info', 'Không sử dụng' + (type == 'attach' ? 'Đính kèm' : 'Hình ảnh'));
	} else if(op == 3) {
...
function getreplycredit() {
	...
	if(real_reply_credit > userextcredit) {
		$('replycredit').innerHTML = '<b class="xi1">Tổng điểm thưởng trả lời quá lớn('+real_reply_credit+')</b>';
	} else {
		if(have_replycredit > 0 && real_reply_credit < 0) {
			$('replycredit').innerHTML = "<font class='xi1'>Quay trở lại"+Math.abs(real_reply_credit)+"</font>";
		} else {
...
function deleteThread() {
	if(confirm('Chắc chắn bạn muốn xoá bài đăng này?') != 0){
		$('delete').value = '1';
		$('postform').submit();
	}
}
