...
function _updatesecqaa(idhash) {
	if($('secqaa_' + idhash)) {
		...
		ajaxget('misc.php?mod=secqaa&action=update&idhash=' + idhash, 'secqaa_' + idhash, null, '', '', function() {
			secST['qaa_' + idhash] = setTimeout(function() {$('secqaa_' + idhash).innerHTML = '<span class="xi2 cur1" onclick="updatesecqaa(\''+idhash+'\')">Đổi Câu hỏi an toàn</span>';}, 180000);
		});
	}
}
	...
	if(isUndefined(play)) {
		if($('seccode_' + idhash)) {
			...
			ajaxget('misc.php?mod=seccode&action=update&idhash=' + idhash, 'seccode_' + idhash, null, '', '', function() {
				secST['code_' + idhash] = setTimeout(function() {$('seccode_' + idhash).innerHTML = '<span class="xi2 cur1" onclick="updateseccode(\''+idhash+'\')">Đổi mã xác nhận</span>';}, 180000);
			});
		}
	} else {
		...
		if(s.substr(0, 7) == 'succeed') {
			...
			if(showmsg) {
				if(type == 'code') {
					...
					showError('Sai Câu hỏi an toàn. Điền lại');
				}
				recall(0);
			}
		}
...
function _copycode(obj) {
	...
	setCopy(BROWSER.ie ? obj.innerText.replace(/\r\n\r\n/g, '\r\n') : obj.textContent, 'Đã copy vào Clipboad');
}
...
function _setCopy(text, msg){
	if(BROWSER.ie) {
		...
		if(r) {
			...
			showDialog('<div class="c"><div style="width: 200px; text-align: center;">Copy thất bại. Hãy chọn "Cho phép truy cập"</div></div>', 'alert');
		}
	} else {
		var msg = '<div class="c"><div style="width: 200px; text-align: center; text-decoration:underline;">Nhấn để Copy vào Clipboad</div>' +
	...
	if(!obj.id) {
		...
		if(!t) {
			...
			if($(t)) {
				...
				s += '<a href="javascript:;" onclick="$(\'' + inpid + '\').value = \'0\'">Thường trực</a>';
...
function _zoom(obj, zimg, nocover, pn) {
	...
	if(!$(menuid) && !pn) {
		...
		menu.id = menuid;
		if(cover) {
			menu.innerHTML = '<div class="zoominner" id="' + menuid + '_zoomlayer" style="display:none"><p><span class="y"><a id="' + menuid + '_imglink" class="imglink" target="_blank" title="Mở cửa sổ mới">Mở cửa sổ mới</a><a id="' + menuid + '_adjust" href="javascipt:;" class="imgadjust" title="Đúng kích thước">Kích thước thực</a>' +
				'<a href="javascript:;" onclick="hideMenu()" class="imgclose" title="Đóng">Close</a></span>Lăn chuột giữa để zoom ảnh</p>' +
...
function _widthauto(obj) {
	...
	if(!CSSLOADED['widthauto'] || $('css_widthauto').disabled) {
		...
		obj.innerHTML = 'Thu hẹp trang';
	} else {
		$('css_widthauto').disabled = true;
		HTMLNODE.className = HTMLNODE.className.replace(' widthauto', '');
		setcookie('widthauto', -1, 86400 * 30);
		obj.innerHTML = 'Mở rộng trang';
	}
	hideMenu();
}
...
function _showCreditmenu() {
	if(!$('extcreditmenu_menu')) {
		...
		menu.innerHTML = '<div class="p_opt"><img src="'+ IMGDIR + '/loading.gif" width="16" height="16" class="vm" /> Loading...</div>';
		$('append_parent').appendChild(menu);
