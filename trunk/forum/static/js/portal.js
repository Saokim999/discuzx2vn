/*
	...
	Translate by Nguyen Trung Kien - Hoangtucoi87 - DCv team
...
function block_delete_item(bid, itemid, itemtype, itemfrom, from) {
	var msg = itemtype==1 ? 'Bạn có chắc sẽ xóa dữ liệu này' : 'Bạn có chắc sẽ xóa dữ liệu này';
	if(confirm(msg)) {
...
function recommenditem_check() {
	...
	if(sel && sel.value) {
		...
		alert("Bạn có chắc sẽ xóa dữ liệu này?");
		return false;
	}
}
...
function toggleSettingShow() {
	if(!$('tbody_setting').style.display) {
		...
		$('a_setting_show').innerHTML = 'Mở rộng thiết lập các mục';
	} else {
		$('tbody_setting').style.display = '';
		$('a_setting_show').innerHTML = 'Thiết lập';
	}
	doane();
}
...
function checkblockname(form) {
	if(!(trim(form.name.value) > '')) {
		showDialog('Tên module không thể để trống', 'error', null, function(){form.name.focus();});
		return false;
	}
	if(form.summary && form.summary.value) {
		...
		if(tag) {
			showBlockSummary();
			form.summary.focus();
			showDialog('Lỗi tùy chỉnh nội dung mã HTML '+tag+' tags không phù hợp', 'error', null, function(){form.summary.select();});
			return false;
		}
	}
		...
		for(var i = 0; i < tags.length; i++) {
			...
			if((!preTag && endTag) || (preTag && !endTag) || preTag.length != endTag.length) {
				if(returnValue) {
					...
					showDialog('Mã số HTML '+tag+' tags không phù hợp', 'error', null, fn, true, fn);
					return false;
				}
			}
		}
...
function blockconver(ele,bid) {
	if(ele && bid) {
		if(confirm('Bạn có chắc muốn chuyển đổi loại module '+ele.options[0].innerHTML+' để '+ele.options[ele.selectedIndex].innerHTML)) {
