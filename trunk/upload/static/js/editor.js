...
buttons[i].onmouseover = function(e) {setEditorTip(editorisfull ? 'Phục hồi kích thước bộ soạn thảo' : 'Chỉnh sửa toàn màn hình');};
	...
	if($(editorid + '_fullswitcher') && BROWSER.ie && BROWSER.ie < 7) {
		$(editorid + '_fullswitcher').onclick = function () {
			showDialog('Trình duyệt của bạn không hỗ trợ, hãy nâng cấp hoặc dùng loại khác', 'notice', 'Lời khuyên');
		};
...
function savedataTime() {
	if(!autosave) {
		$(editorid + '_svdsecond').innerHTML = '<a title="Ấn vào để mở tự lưu" href="javascript:;" onclick="setAutosave()">Mở tự động lưu</a> ';
		return;
	}
	if(!savedatac) {
		savedatac = savedataInterval;
	...
	$(editorid + '_svdsecond').innerHTML = '<a title="Ấn để tắt tự lưu" href="javascript:;" onclick="setAutosave()">' + savedatac + ' giây lưu</a> ';
	savedatac -= 10;
}

function setAutosave() {
	autosave = !autosave;
	setEditorTip(autosave ? 'Lưu tự động được bật' : 'Lưu tự động tắt');
...
function showEditorMenu(tag, params) {
	...
	if(menu) {
		...
		switch(tag) {
			case 'url':
				str = 'Nhập URL:<br /><input type="text" id="' + ctrlid + '_param_1" style="width: 98%" value="" class="px" />'+
					(selection ? '' : '<br />Nhập Liên kết:<br /><input type="text" id="' + ctrlid + '_param_2" style="width: 98%" value="" class="px" />');
				break;
			case 'forecolor':
			...
			case 'free':
				...
				var lang = {'quote' : 'Nhập trích dẫn', 'code' : 'Nhập văn bản', 'hide' : 'Nhập nội dung muốn ẩn', 'free' : 'Nếu bạn dùng chức năng bán chủ đề thì có thể dùng cái này cho việc giới thiệu'};
				...
				break;
			case 'tbl':
				str = '<p class="pbn">Số hàng: <input type="text" id="' + ctrlid + '_param_1" size="2" value="2" class="px" /> &nbsp; Số cột: <input type="text" id="' + ctrlid + '_param_2" size="2" value="2" class="px" /></p><p class="pbn">Chiều rộng bảng: <input type="text" id="' + ctrlid + '_param_3" size="2" value="" class="px" /> &nbsp; Màu nền: <input type="text" id="' + ctrlid + '_param_4" size="2" class="px" onclick="showColorBox(this.id, 2)" /></p><p class="xg2 pbn" style="cursor:pointer" onclick="showDialog($(\'tbltips_msg\').innerHTML, \'notice\', \'Mẹo\', null, 0)"><img id="tbltips" title="Lời khuyên" class="vm" src="' + IMGDIR + '/info_small.gif"> Mẹo viết nhanh</p>';
				str += '<div id="tbltips_msg" style="display: none">"[tr=Màu]” đường viền<br />"[td=Chiều rộng]” Xác định chiều rộng cột<br />"[td=Cột,Hàng,Chiều rộng]” Chiều rộng<br /><br />Ví dụ viết nhanh<div class=\'xs0\' style=\'margin:0 5px\'>[table]<br />Name:|Friendsky<br />Version:|FS<br />[/table]</div>Với"|”Tách mỗi cột, bảng nếu"|"Với"\\|"Thay vào đó, sử dụng dòng"\\n"Thay vào đó.</div>';
				break;
			case 'aud':
				str = '<p class="pbn">Xin vui lòng nhập địa chỉ file nhạc:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="xg2 pbn">Link có đuôi wma mp3 ra rm<br />Ví dụ: http://server/audio.wma</p>';
				break;
			case 'vid':
		...
		if(menupos == '00') {
			...
			s = '<div class="p_opt cl"><span class="y" style="margin:-10px -10px 0 0"><a onclick="hideMenu();return false;" class="flbc" href="javascript:;">关闭</a></span><div>' + str + '</div><div class="pns mtn"><button type="submit" id="' + ctrlid + '_submit" class="pn pnc"><strong>OK</strong></button></div></div>';
		}
		menu.innerHTML = s;
