...
if(!file_exists($sqlfile)) {
	show_msg('file SQL '.$sqlfile.' không tồn tại');
}
...
if($_POST['delsubmit']) {
	...
	show_msg('Xóa bảng và các thao tác hoàn thành', $theurl.'?step=style');
}
...
if($_GET['step'] == 'start') {
	...
	if(DB::fetch_first("SHOW COLUMNS FROM ".DB::table('forum_activityapply')." LIKE 'contact'")) {
		$query = DB::query("UPDATE ".DB::table('forum_activityapply')." SET message=CONCAT_WS(' Liên hệ:', message, contact) WHERE contact<>''");
	...
	if(empty($newtables) || empty($newsqls)) {
		show_msg('Nội dung của tập tin SQL có sản phẩm nào, hãy chắc chắn');
	}

	$i = empty($_GET['i'])?0:intval($_GET['i']);
	$count_i = count($newtables);
	if($i>=$count_i) {
		show_msg('Nâng cấp cấu trúc cơ sở dữ liệu hoàn tất, nhập vào các dữ kiện cập nhật dữ liệu tiếp theo', $theurl.'?step=data');
	}
	...
	if(!$query = DB::query("SHOW CREATE TABLE ".DB::table($newtable), 'SILENT')) {
			...
			if($key == 'PRIMARY') {
				if($value != $oldcols[$key]) {
					if(!empty($oldcols[$key])) {
						...
						if(!DB::query($usql, 'SILENT')) {
							show_msg('Nâng cấp bảng '.DB::table($newtable).' lỗi, hãy thực hiện các thao tác cập nhật sau đây bằng tay, sau đó chạy lại chương trình nâng cấp:<br><br><b> Hiện trạng nâng cấp SQL </b>:<div style=\"position:absolute;font-size:11px;font-family:verdana,arial;background:#EBEBEB;padding:0.5em;\">'.dhtmlspecialchars($usql)."</div><br><b>Error</b>: ".DB::error()."<br><b>Errno.</b>: ".DB::errno());
						} else {
							$msg = 'Đổi tên bảng '.DB::table($newtable).' thành công！';
		...
		if(!empty($updates)) {
			...
			if(!DB::query($usql, 'SILENT')) {
				...
				$msg = 'Nâng cấp bảng '.DB::table($newtable).' hoàn thành！';
			}
		} else {
			$msg = 'Kiểm tra danh sách '.DB::table($newtable).' hoàn thành, không cần nâng cấp, bỏ qua';
		}
	}
	...
	if(empty($_GET['op']) || $_GET['op'] == 'realname') {
		...
		if($i==0) {
			...
			if(!empty($value)) {
				show_msg("Nâng cấp tên tính năng hoàn tất", "$theurl?step=data&op=$nextop");
			}
		...
		if($n>0) {
			show_msg("Tên tính năng nâng cấp thực sự [$n/$t]", "$theurl?step=data&op=realname&i=$n");
		} else {
			show_msg("Nâng cấp tên tính năng hoàn tất", "$theurl?step=data&op=$nextop");
		}
		...
		if(empty($settings['reglinkname'])) {
			DB::query("REPLACE INTO ".DB::table('common_setting')." VALUES ('reglinkname', 'Đăng ký')");
		}
		...
		if(!isset($settings['focus'])) {
			$focusnew = array('title' => 'Webmaster khuyến nghị', 'cookie' => 1);
		...
		if(!isset($settings['article_tags'])) {
			$article_tagsnew = addslashes(serialize(array(1 => 'gốc', 2 => 'điểm nóng', 3 => 'Hình ảnh', 4 => 'Broke', 5 => 'Headlines', 6 => 'slide', 7 => 'cuộn', 8 => 'Đề nghị')));
		...
		show_msg("Nâng cấp mục cấu hình thành công", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp thiết lập quản lý nhóm hoàn tất", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp rao vặt", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp nhiệm vụ hoàn thành", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp báo cáo hoàn tất", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp chuyển vùng thống kê Ứng Dụng", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp chuyển hướng dữ liệu", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp hệ thống diễn đàn thành công", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp thành công Đề Nghị Trả Lời", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp người đùng thành công", "$theurl?step=data&op=$nextop");
		...
		show_msg("Mã tùy chỉnh cho phép nâng cấp hoàn tất", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp con dấu hoàn tất", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nnâng cấp các module thu nhỏ hình hoàn tất", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp bình luận bản đầy đủ", "$theurl?step=data&op=$nextop");
		...
		show_msg("nâng cấp bản đồ bài viết hoàn tất", "$theurl?step=data&op=$nextop");
		...
		if(!empty($aa[1])) {
			...
			DB::query("UPDATE  `pre_common_block_style` SET name = replace(`name`, 'X1.5', 'nội bộ')");
			DB::query("UPDATE  `pre_common_block_style` SET name = replace(`name`, 'X2.0', 'nội bộ')");
		}
		show_msg("Nâng cấp Module mẫu hoàn tất", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp các tập lệnh module thành công", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp các điều khoản của nhóm người dùng hoàn tất", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp nhóm trang chủ hoàn thành", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp các thiết lập tên miền hoàn tất", "$theurl?step=data&op=$nextop");
		...
		show_msg("nâng cấp quyền xem hình ảnh hoàn tất", "$theurl?step=data&op=$nextop");
		...
		if($updateverify) {
			...
			if($t) {
				...
				if($n) {
					show_msg("Nâng cấp xác thực tên thật[$n/$t]", "$theurl?step=data&op=verify&i=$n&updateverify=true");
				}
			}
		}
		show_msg("Nâng cấp dữ liệu xác thực hoàn tất", "$theurl?step=data&op=$nextop");
		...
		if($needupgrade && $count) {
			...
			if(DB::num_rows($query)) {
				...
				$start += $limit;
				show_msg("Nâng cấp bảng đính kèm diễn đàn ... $start/$count", "$theurl?step=data&op=forumattach&start=$start");
			}
		...
		show_msg("Nâng cấp hoàng tất bảng đính kèm của diễn đàn", "$theurl?step=data&op=$nextop");
		...
		if($needupgraded) {
			...
			if(!file_exists($cachefile)) {
				...
				if($data && @$fp = fopen($cachefile, 'w')) {
					...
					show_msg("Bảng chủ đề không thể xử lý hình ảnh, bỏ qua", "$theurl?step=data&op=$nextop");
				}
			} else {
				$data = @file($cachefile);
				if(!$data) {
			...
			if(!$tids) {
				@unlink($cachefile);
				show_msg("Bảng xử lý hình ảnh chủ đề", "$theurl?step=data&op=$nextop");
			}
			...
			$start += $limit;
			show_msg("Bảng xử lý hình ảnh chủ đề ... $start ", "$theurl?step=data&op=threadimage&start=$start");
		} else {
			show_msg("Bảng hình ảnh chủ đề không thể xử lý, bỏ qua", "$theurl?step=data&op=$nextop");
		}
		...
		show_msg("Nâng cấp hoàn tất các dữ liệu kiểm toán", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp hoàn tất, người sáng lập dữ liệu", "$theurl?step=data&op=$nextop");
		...
		show_msg("dữ liệu gói ngôn ngữ Plug nâng cấp hoàn tất", "$theurl?step=data&op=$nextop");

	} else {

		$deletevar = array('app', 'home');//config :thư mục cần thiết loại bỏ 
		...
		if(save_config_file($configfile, $_config, $default_config, $deletevar)) {
			show_msg("Xử lý dữ liệu hoàn tất", "$theurl?step=delete");
		} else {
			show_msg('"config/config_global.php" File cần được cập nhật trong thư mục "config/" không thể ghi, sử dụng phần mềm FTP để cấp quyền lại cho thư mục "config/" .<br /><br /><a href="'.$theurl.'?step=delete">Khi bạn hoàn thành bấm vào đây để tiếp tục</a>');
		}
	}
	...
	if(!$devmode) {
		show_msg("Xóa xử lý dữ liệu, bước tiếp theo", "$theurl?step=style");
	}
	...
	if($deltables) {
		...
		$deltablehtml .= '</table>';
		echo "<p>Phần dữ kiện <strong>sau đây</strong> so với cơ sở dữ liệu tiêu chuẩn là không cần thiết:<br> Bạn có thể quyết định xem sự cần thiết để loại bỏ </p>$deltablehtml";
	}
			...
			foreach ($cols as $coltype => $col) {
				if (is_array($col)) {
					foreach ($col as $index => $indexvalue) {
						$delcolumnhtml .= "<tr><td><input type=\"checkbox\" name=\"delcols[$tablename][$coltype][$index]\" value=\"1\"></td><td>{$config['tablepre']}$tablename</td><td> Chỉ số($coltype) $index $indexvalue</td></tr>";
					}
				} else {
					$delcolumnhtml .= "<tr><td><input type=\"checkbox\" name=\"delcols[$tablename][$col]\" value=\"1\"></td><td>{$config['tablepre']}$tablename</td><td>Lĩnh vực $col</td></tr>";
				}
			}
		}
		$delcolumnhtml .= '</table>';

		echo "<p>Lĩnh vực <strong> sau đây</strong> So với cơ sở dữ liệu tiêu chuẩn là không cần thiết:<br>Bạn có thể quyết định xem có cần thiết để loại bỏ</p>$delcolumnhtml";
	}

	if(empty($deltables) && empty($delcolumns)) {
		echo "<p>So với cơ sở dữ liệu tiêu chuẩn, không có các bảng dữ liệu và các lĩnh vực cần phải xoá</p><a href=\"$theurl?step=style\"> Hảy nhấn Next</a></p>";
	} else {
		echo "<p><input type=\"submit\" name=\"delsubmit\" value=\"Delete Submit\"></p><p>Bạn cũng có thể bỏ qua các bảng phụ và lĩnh vực <br><a href=\"$theurl?step=style\">Tới bước tiếp theo</a></p>";
	}
	echo '</form>';
	...
	if(empty($_GET['confirm'])) {
		show_msg("Vui lòng xác nhận rằng bạn muốn khôi phục lại phong cách mặc định？<br /><br /><a href=\"$theurl?step=style&confirm=yes\">[ Ok ]</a>&nbsp;&nbsp;<a href=\"$theurl?step=cache\">[không ]</a>", '');
	}
	...
	show_msg("Các phong cách mặc định đã được phục hồi, bước tiếp theo", "$theurl?step=cache");
	...
	show_msg('<span id="finalmsg">cập nhập Cache，Vui lòng đợi ...</span><iframe src="../misc.php?mod=initsys" style="display:none;" onload="document.getElementById(\'finalmsg\').innerHTML = \'Xin chúc mừng, nâng cấp cấu trúc cơ sở dữ liệu hoàn tất! Để bảo mật dữ liệu, hãy xóa tập tin này.\'"></iframe>');
...
function show_msg($message, $url_forward='') {
	...
	if($url_forward) {
		$message = "<a href=\"$url_forward\">$message (Chuyển trong...)</a><script>setTimeout(\"window.location.href ='$url_forward';\", 1);</script>";
	}
...
function show_header() {
	...
	* {font-size:12px; font-family: Verdana, Arial, Helvetica, sans-serif; line-height: 1.5em; word-break: break-all; }
	...
	<body>
	<div class="bodydiv">
	<h1>Công cụ nâng cấp cơ sở dữ liệu</h1>
	<div style="width:90%;margin:0 auto;">
	<table id="menu">
	<tr>
	<td{$nowarr[start]}>Nâng cấp bắt đầu</td>
	<td{$nowarr[sql]}>Thêm và cập nhật cấu trúc cơ sở dữ liệu</td>
	<td{$nowarr[data]}>cập nhật dữ liệu</td>
	<td{$nowarr[delete]}>Xóa các cấu trúc cơ sở dữ liệu</td>
	<td{$nowarr[cache]}>nâng cấp hoàn tất</td>
	</tr>
	</table>
	<br>
END;
